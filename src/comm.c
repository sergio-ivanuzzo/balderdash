...
{
    char buf[MSL];
    char escaped_argument[MAX_INPUT_LENGTH * 4 + 1];
    const char *src;
    char *dst;
    size_t remaining;

    buf[0] = '\0';
    escaped_argument[0] = '\0';

    if (argument[0] != '\0')
	{
	    /*
	     * popen() uses the shell; escape single quotes so user input cannot
	     * terminate the quoted string and inject extra shell commands.
	     */
	    src = argument;
	    dst = escaped_argument;
	    remaining = sizeof(escaped_argument) - 1;
    ...
    strncat(buf, "gdb rom rom.core", sizeof(buf) - strlen(buf) - 1);
    p_open(ch, buf);
