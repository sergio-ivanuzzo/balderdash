...
void p_open(CHAR_DATA *ch, char *argument)
{
    ...
    if (fp == NULL)
    {
        bugf("p_open: popen failed: %s", argument);
        if (ch != NULL)
            send_to_char("Execution error.\n\r", ch);
        return;
    }

    if (fgetf(buf, 5000, fp) == NULL)
        buf[0] = '\0';
...
void p_openw(char *command, char *argument)
{
    ...
    if (command == NULL || command[0] == '\0' || argument == NULL)
        return;
    ...
    if (fp == NULL)
    {
        bugf("p_openw: popen failed: %s", command);
        return;
    }
...
void do_gdb(CHAR_DATA *ch, char *argument)
{
    if (argument[0] != '\0')
    {
        send_to_char("For security reasons gdb arguments are disabled.\n\r", ch);
        return;
    }
...
void mailing(char *to, char *subj, char *body)
{
	...
	char buf[MAX_STRING_LENGTH];
	char safe_to[MAX_INPUT_LENGTH];
	char safe_subj[MAX_INPUT_LENGTH];
	int i, j;

	for (i = 0, j = 0; to != NULL && to[i] != '\0' && j < (int) sizeof(safe_to) - 1; i++)
	{
		if (to[i] == '\n' || to[i] == '\r')
			continue;
		safe_to[j++] = to[i];
	...
	for (i = 0, j = 0; subj != NULL && subj[i] != '\0' && j < (int) sizeof(safe_subj) - 1; i++)
	{
		if (subj[i] == '\n' || subj[i] == '\r')
			continue;
		safe_subj[j++] = subj[i];
	...
	sprintf(buf, 	"To: %s\n"
			...
			"Subject: %s\n\n%s",
			safe_to, safe_subj, body == NULL ? "" : body);
	p_openw(mailer, buf);
}
