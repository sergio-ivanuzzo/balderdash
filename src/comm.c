...
void p_open(CHAR_DATA *ch, char *argument)
{
    ...
    {
        if (ch != NULL)
            send_to_char("Unable to execute command.\n\r", ch);
        return;
    }
...
static bool has_shell_meta_chars(const char *argument)
{
...
void do_gdb(CHAR_DATA *ch, char *argument)
{
    ...
    if (argument[0] != '\0')
    {
        if (has_shell_meta_chars(argument))
        {
            send_to_char("Refused: unsafe characters in gdb command.\n\r", ch);
            return;
        }

        snprintf(buf, sizeof(buf), "echo '%s' | ", argument);
    }
