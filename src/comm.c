...
void p_open(CHAR_DATA *ch, char *argument)
{
    ...
    for (i = 0; i < (int)(sizeof(allowed_commands) / sizeof(allowed_commands[0])); i++)
    {
        if (!str_cmp(argument, allowed_commands[i]))
        {
            allowed = TRUE;
            break;
        }
    ...
    if (!allowed)
    {
        bugf("Blocked unsafe p_open command: %s", argument);
        if (ch != NULL)
            send_to_char("Command blocked by security policy.\n\r", ch);
        return;
    }
    ...
    {
        if (ch != NULL)
            send_to_char("Command execution failed.\n\r", ch);
        return;
    }
...
void p_openw(char *command, char *argument)
{
    ...
    {
        bugf("Blocked unsafe p_openw command: %s", command);
        return;
    }
