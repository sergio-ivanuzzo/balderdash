...
void do_unread(CHAR_DATA *ch, char *argument)
{
    ...
    {
	found = TRUE;
	sprintf(buf, "Ó òåáÿ %d %s ", count, hours(count, TYPE_UNREAD));
	...
	sprintf(buf, "%d %s ", count, hours(count, TYPE_IDEAS)); 
    	...
    	strcat(buf, count == 1 ? " ÷üþ-òî " : " ÷üè-òî ");
    	strcat(buf, count == 1 ? "ñâåòëóþ " : "ñâåòëûå ");
    	strcat(buf, count == 1 ? "ãîëîâó" : "ãîëîâû");
    	strcat(buf, ".\n\r");
	send_to_char(buf, ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf, "%d %s áûëî äîáàâëåíî.\n\r",
	...
	for (count = 1; count < MAX_CLAN; count++) 
	{
		...
		if (stamp < pnote->date_stamp
		    ...
		    && (is_exact_name("êëàíó", pnote->to_list)
			|| is_exact_name("êëàí", pnote->to_list)
		...
		if (ch->level < MAX_LEVEL)
		    sprintf(buf, "\n\rÂ òâîåì êëàíå óæå áîëåå %d íåäåëü"
			    " îòñóòñòâóþò íîâîñòè.", CLAN_NEWS_EXPIRED);
		else
		    sprintf(buf, "\n\rÂ êëàíå %s óæå áîëåå %d íåäåëü"
			    " îòñóòñòâóþò íîâîñòè.",
	...
	send_to_char("Ó òåáÿ íåò íåïðî÷èòàííûõ ñîîáùåíèé.\n\r", ch);
...
{
    fprintf(fp, "Îòïðàâèòåëü: %s~\n", pnote->sender);
    fprintf(fp, "Äàòà: %s~\n", pnote->date);
    fprintf(fp, "Stamp: %ld\n", (long)pnote->date_stamp);
    fprintf(fp, "Êîìó: %s~\n", pnote->to_list);
    fprintf(fp, "Òåìà: %s~\n", pnote->subject);
    fprintf(fp, "Ñîîáùåíèå:\n%s~\n", pnote->text);
...
void load_thread(char *name, NOTE_DATA **list, int type, time_t free_time)
{
	...
	do
	{
        ...
        if (str_cmp(fread_word(fp), "Îòïðàâèòåëü:"))
            break;

	free_string(pnote->sender);
        pnote->sender = fread_string(fp);
 
        if (str_cmp(fread_word(fp), "Äàòà:"))
            break;
	...
	free_string(pnote->date);
        ...
        if (str_cmp(fread_word(fp), "Êîìó:"))
            break;

	free_string(pnote->to_list);
        pnote->to_list = fread_string(fp);
 
        if (str_cmp(fread_word(fp), "Òåìà:"))
            break;

	free_string(pnote->subject);
        pnote->subject = fread_string(fp);
 
        if (str_cmp(fread_word(fp), "Ñîîáùåíèå:"))
            break;
...
bool is_note_to(CHAR_DATA *ch, NOTE_DATA *pnote)
{
	...
	|| is_exact_name("âñå", pnote->to_list)
	|| is_exact_name("âñåì", pnote->to_list))
    {
	return TRUE;
    }
	...
	&& (is_exact_name("immortal", pnote->to_list)
	    ...
	    || is_exact_name("áîãàì", pnote->to_list)
	    || is_exact_name("èììàì", pnote->to_list)))
    {
	return TRUE;
    }
	...
	&& (is_exact_name("ñåìüå", pnote->to_list)
	    || is_exact_name("ñóïðóãó", pnote->to_list)
	    || is_exact_name("ñóïðóãå", pnote->to_list)
	    || is_exact_name("ìóæó", pnote->to_list)
	    || is_exact_name("æåíå", pnote->to_list)
...
void parse_note(CHAR_DATA *ch, char *argument, int type)
{
    ...
    {
	send_to_char("Òåáÿ óñûïèëè, òû íå ìîæåøü â òàêîì ñîñòîÿíèè ðàáîòàòü ñ ñîîáùåíèÿìè.\n\r", ch);
	return;
    } 
    ...
    if (arg[0] == '\0'
	|| !str_prefix(arg, "read")
	|| !str_prefix(arg, "ïðî÷èòàòü")
	|| !str_prefix(arg, "÷èòàòü"))
    {
        bool fAll;
 
        if (!str_cmp(argument, "all") || !str_cmp(argument, "âñå"))
        {
            fAll = TRUE;
            anum = 0;
        }
	    ...
	    sprintf(buf, "Ó òåáÿ áîëüøå íåò íåïðî÷èòàííûõ %s.\n\r", list_name);
	    send_to_char(buf, ch);
            return;
        }
        ...
        {
            send_to_char("×èòàòü êàêîé íîìåð?\n\r", ch);
            return;
        }
	...
	sprintf(buf, "Òåáå åùå íå íàïèñàëè ñòîëüêî %s.\n\r", list_name);
	send_to_char(buf, ch);
        return;
    }

    if (!str_prefix(arg, "list") || !str_prefix(arg, "ñïèñîê"))
    {
        char *str;
        bool found = FALSE;
        char last_stamp[MSL];
	...
	if (!found)
	{
	    ...
	    {
	    case NOTE_NOTE:	
		send_to_char("Äëÿ òåáÿ íåò ïèñåì.\n\r", ch);
		break;
	    case NOTE_IDEA:
	      	send_to_char("Äëÿ òåáÿ íåò èäåé.\n\r", ch);
    		break;
	    case NOTE_PENALTY:
		send_to_char("Äëÿ òåáÿ íåò íåïðî÷èòàíûõ íàêàçàíèé.\n\r", ch);
		break;
	    case NOTE_NEWS:
		send_to_char("Äëÿ òåáÿ íåò íîâîñòåé.\n\r", ch);
		break;
	    case NOTE_CHANGES:
	      	send_to_char("Äëÿ òåáÿ íåò èçìåíåíèé.\n\r", ch);
    		break;
	    case NOTE_VOTES:
	      	send_to_char("Äëÿ òåáÿ íåò íîâûõ îïðîñîâ.\n\r", ch);
    		break;
	    }
	}
	return;
    ...
    if (!str_prefix(arg, "autor") || !str_prefix(arg, "àâòîð"))
    {
        char *str;
        bool found = FALSE;
        char last_stamp[MSL];
    ...
    if (!str_prefix(arg, "theme") || !str_prefix(arg, "çàãîëîâîê"))
    {
        char *str;
        bool found = FALSE;
        char last_stamp[MSL];
		...
		if (IS_NULLSTR(arg))
		{
			send_to_char("Ââåäè ñëîâà äëÿ ïîèñêà â òåìàõ ïèñåì.\n\r", ch);
			return;
		}
    ...
    if (!str_prefix(arg, "remove") || !str_prefix(arg, "óäàëèòü"))
    {
        if (!is_number(argument))
        {
            send_to_char("Óäàëèòü ñîîáùåíèå ïîä êàêèì íîìåðîì?\n\r", ch);
            return;
        }
    ...
    if ((!str_prefix(arg, "delete") || !str_prefix(arg, "óíè÷òîæèòü"))
    ...
    if (!str_prefix(arg, "catchup") || !str_prefix(arg, "ïîìåòèòü"))
    {
	switch(type)
	{
	case NOTE_NOTE:
    ...
    {
	sprintf(buf, "Ó òåáÿ íåäîñòàòî÷íûé óðîâíü äëÿ ðåäàêòèðîâàíèÿ %s.\n\r",
		list_name);
	send_to_char(buf, ch);
	return;
    }
	...
	{
	    send_to_char("Áîãè çàïðåòèëè òåáå ïèñàòü ñîîáùåíèÿ.\n\r", ch);
	    return;
	}
	...
	{
	    send_to_char("Òû óæå çàíèìàåøüñÿ äðóãèì ñîîáùåíèåì.\n\r", ch);
	    return;
	}

	if (strlen(ch->pnote->text) + strlen(argument) >= 4096)
	{
	    send_to_char("Ñîîáùåíèå ñëèøêîì äëèííîå.\n\r", ch);
	    return;
	}
	...
	{
	    send_to_char("Áîëüøå íåò ñòðîê äëÿ óäàëåíèÿ.\n\r",ch);
	    return;
	}
    ...
    if (!str_prefix(arg, "subject") || !str_prefix(arg, "òåìà"))
    {
	if (IS_SET(ch->comm, COMM_NONOTES))
	{
	    send_to_char("Áîãè çàïðåòèëè òåáå ïèñàòü ñîîáùåíèÿ.\n\r", ch);
	...
	note_attach(ch, type);
        if (ch->pnote->type != type)
        {
            send_to_char("Òû óæå çàíèìàåøüñÿ äðóãîé çàïèñêîé.\n\r", ch);
            return;
        }
    ...
    if (!str_prefix(arg, "to") || !str_prefix(arg, "êîìó"))
    {
	...
	note_attach(ch, type);
        ...
        if (!str_cmp(argument, "êëàíó")
	    || !str_cmp(argument, "êëàí")
	    || !str_cmp(argument, "clan"))
        {
	    if (!is_clan(ch) || IS_INDEPEND(ch))
	    {
		send_to_char("Íî âåäü òû æå íå â êëàíå!\n\r", ch);
		return;
	    }	    
		...
		{		
			...
			|| (!str_cmp(argument, "áîãè")
	    	|| !str_cmp(argument, "áîãàì")
	    	|| !str_cmp(argument, "immortal")))
        	{
			}
			else
			{
				send_to_char("Ïîêà òû íå ïðîøåë îäîáðåíèå, òû ìîæåøü ïèñàòü ëèøü áîãàì. Íàáåðè ? {yîäîáðåíèå{x.\n\r", ch);
				return;
	    	}
		    strcpy(buf, argument);
		}
    ...
    if (!str_prefix(arg, "clear") || !str_prefix(arg, "ñòåðåòü"))
    {
	if (ch->pnote != NULL)
	{
	    free_note(ch->pnote);
    ...
    if (!str_prefix(arg, "show") || !str_prefix(arg, "ïîêàçàòü"))
    {
	if (ch->pnote == NULL)
	{
	    send_to_char("Òû åùå ïîêà íå ïèøåøü íèêàêèõ ñîîáùåíèé.\n\r", ch);
	    return;
	}

	if (ch->pnote->type != type)
	{
	    send_to_char("Òû ðàáîòàåøü íàä äðóãèì òèïîì ñîîáùåíèÿ.\n\r", ch);
	    return;
	}
...
/*	sprintf(buf, "%s: %s\n\rÊîìó: %s\n\r",
	...
	if (ch->pnote == NULL)
	{
	    printf_to_char("Òû æå åùå íè÷åãî íå íàïèñàë%s.\n\r", ch, SEX_ENDING(ch));
	    return;
	}

        if (ch->pnote->type != type)
        {
            send_to_char("Òû ðàáîòàåøü íàä äðóãèì òèïîì çàïèñêè.\n\r", ch);
            return;
        }

	if (!str_cmp(ch->pnote->to_list, ""))
	{
	    send_to_char("Íàäî óêàçàòü ïîëó÷àòåëÿ (èìÿ, âñåì, Áîãàì, "
			 "ñåìüå, êëàíó).\n\r", ch);
	    return;
	}
	...
	{
	    send_to_char("Íåîáõîäèìî óêàçàòü òåìó ïèñüìà.\n\r", ch);
	    return;
	}

	if (ch->pnote->type == NOTE_VOTES && ch->pnote->vote == NULL)
	{
	    send_to_char("Íåîáõîäèìî ñîçäàòü õîòÿ áû îäèí âàðèàíò îòâåòà.\n\r", ch);
	    return;
	}
	...
	send_to_char("Òâîå ñîîáùåíèå ïîñëàíî àäðåñàòàì.\n\r", ch);
    ...
    if (type == NOTE_VOTES)
    {
        if (!str_prefix(arg, "âàðèàíò") || !str_prefix(arg, "variant"))
        {
	    char buf[MIL];
	    VOTE_DATA *vd;
	    ...
	    note_attach(ch, type);
            ...
            if (!str_cmp(buf, "-"))
            {
                if ((vd = ch->pnote->vote) != NULL)
                {
                    ...
                    send_to_char("Íå ñîçäàíî åùå íè îäíîãî âàðèàíòà.\n\r", ch);

                return;
            }
        }

        if (!str_prefix(arg, "ïðîãîëîñîâàòü") || !str_prefix(arg, "vote") || !str_prefix(arg, "ãîëîñîâàòü"))
        {
            ...
            if (!is_number(arg))
            {
                send_to_char("Àðãóìåíòû äîëæíû áûòü ÷èñëîâûìè.\n\r", ch);
                return;
            }
            num = atoi(arg);
                ...
                if (is_note_to(ch, pnote) && vnum++ == num)
                {
                    ...
                    for (vd = pnote->vote; vd; vd = vd->next)
                    {
                        ...
                        for (cv = vd->votes; cv; cv = cv->next)
                            if (cv->char_id == ch->id)
                            {
                                printf_to_char("Òû óæå ó÷àñòâîâàë%s â ýòîì îïðîñå.\n\r", ch, SEX_ENDING(ch));
                                return;
                            }
                    }
                    ...
                    if (!vd_curr)
                    {
                        send_to_char("Íåò òàêîãî âàðèàíòà îòâåòà.\n\r", ch);
                        return;
                    }
    ...
    send_to_char("Òû íå ìîæåøü ýòîãî ñäåëàòü.\n\r", ch);
    return;
}
