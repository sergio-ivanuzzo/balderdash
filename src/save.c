...
static bool decompress_gzip_to_file args((char *gz_path, char *out_path));

double get_explore_percent(CHAR_DATA *ch);

static bool decompress_gzip_to_file(char *gz_path, char *out_path)
{
    gzFile gzfp;
    FILE *out_fp;
    char buffer[4096];
    ...
    gzfp = gzopen(gz_path, "rb");
    if (gzfp == NULL)
        return FALSE;

    out_fp = file_open(out_path, "wb");
    if (out_fp == NULL)
    {
        gzclose(gzfp);
        return FALSE;
    ...
    while ((bytes_read = gzread(gzfp, buffer, sizeof(buffer))) > 0)
    {
        if (fwrite(buffer, 1, (size_t) bytes_read, out_fp) != (size_t) bytes_read)
        {
            file_close(out_fp);
            gzclose(gzfp);
            unlink(out_path);
    ...
    if (unlink(gz_path) != 0)
    {
        bugf("Load_char_obj: failed to remove %s: %s", gz_path, strerror(errno));
    }
...
/*    sprintf(strsave, "%s%s/%s", PLAYER_DIR, ch->pcdata->email, capitalize(ch->name));*/
	...
	//	ch->pcdata->obj_count_out = sobj_out;
		...
		/* Ñîõðàíÿòü òîëüêî òåõ ëîøàäåé, êîòîðûõ â ìèðå áîëåå 1-ãî ýêçåìïëÿðà */
...
/*
 * #Ñîõðàíåíèå ÷àðà â ôàéë #save #char_save
 */
...
//    fprintf(fp, "ObjCountOut  %d\n", ch->pcdata->obj_count_out);
    ...
    //ïîñòàâèòü ïðîâåðêó - ïðîâåðî÷íîå âðåìÿ ìåíüøå èëè íåò...
...
void fwrite_obj(CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest)
{
    ...
    if (obj->next_content != NULL){
        ...
        // Íåîáõîäèìî äëÿ òîãî, ÷òîáû â ôàéë íå ñîõðàíÿëèñü îáúåêòû
        // ñ äðóãèõ ñóíäóêîâ. Ò.å. ïåðâûé ýëåìåíò ôàéëà - îáúåêò ñóíäóê (iNest = 0)
        // Ó îñòàëüíûõ îáúåêòîâ iNest > 0 óâåëè÷èâàåòñÿ.
        } else {
...
//    sobj_out++;
    ...
    /* [10847] Îáåðåã ïðè âñòàâëåííîì àðòåôàêòå êðèâî çàïèñûâàþòñÿ ýôôåêòû */
...
bool load_char_obj(DESCRIPTOR_DATA *d, char *name, bool beac, bool equip){
    ...
    ch->pcdata->lastip			= str_dup("íåèçâåñòåí");
    ch->pcdata->lasthost		= str_dup("íåèçâåñòåí");
    ch->pcdata->genip			= str_dup("íåèçâåñòåí");
    ...
    sprintf(buf, "%s.gz", strsave);

    if ((fp = file_open(buf, "r")) != NULL)
    {
        file_close(fp);

        if (!decompress_gzip_to_file(buf, strsave))
            bugf("Load_char_obj: failed to decompress %s", buf);
    }
...
/* ÇÀÏÈØÅÌ Â ËÎÃ IP âîøåäøåãî ÷àðà, åãî íèê, åãî ïî÷òó, âðåìÿ è äàòó*/
    if (equip) {
...
/* ÇÀÃÐÓÆÀÅÌ ÏÀÐÀÌÅÒÐÛ ×ÀÐÀ ÈÇ ÔÀÉËÀ */
...
//	sobj_in++;
	...
	case 'E':
		...
		else
		{
		    ...
		    {
			if (obj->carried_by)
	    			bugf("óíè÷òîæàåì îáúåêò %s ó ÷àðà/ìîáà %s.",obj->name,obj->carried_by->name);
...
Purpose:	Ôóíêöèÿ óäàëåíèÿ òåìïîâûõ ñóíäóêîâ.
 ****************************************************************************/
...
//    bugf("Óäàëåíî òåìïîâûõ ñóíäóêîâ %d!", count);
...
/*****************************************************************************
Name:		save_chest
Purpose:	Ôóíêöèÿ ñîõðàíåíèÿ îáúåêòîâ ñóíäóê â ôàéë.
...
void save_chest(long id)
{
	...
	if (obj->item_type == ITEM_CHEST && obj->in_room != NULL)
	{
	    if (id == 0)
	    {
		save_one_chest(obj);	// Ñîõðàíÿåì âñå
		count++;
	    }
	    else if (obj->id == id)
	    {
		save_one_chest(obj);	// Ñîõðàíÿåì êîíêðåòíûé ñóíäóê
	    }
	}
    }
    if (id == 0)
	bugf("Ñîõðàíåíî ñóíäóêîâ %d!", count);
    delete_temp_chest();	// Óäàëÿåì òåìïîâûå ñóíäóêè
    return;
}

/*****************************************************************************
Name:		save_one_chest
Purpose:	Ôóíêöèÿ ñîõðàíåíèÿ îäíîãî ñóíäóêà â ôàéë.
...
/*****************************************************************************
Name:		load_chest
Purpose:	Ôóíêöèÿ ÷òåíèÿ ôàéëîâ-ñóíäóêîâ.
...
/*****************************************************************************
Name:		delete_chest
Purpose:	Ôóíêöèÿ óäàëåíèÿ îäíîãî ñóíäóêà.
