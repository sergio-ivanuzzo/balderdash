...
static bool safe_decompress_player_file(const char *archive_path)
{
    pid_t pid;
    int status;
...
bool load_char_obj(DESCRIPTOR_DATA *d, char *name, bool beac, bool equip){
    ...
    {
	if (!safe_decompress_player_file(strsave))
	    bugf("Load_char_obj: failed to decompress %s", strsave);
    }
    file_close(fp);
