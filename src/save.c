...
static bool decompress_player_gzip(const char *path)
{
    pid_t pid;
    int status;
    ...
    if (pid < 0)
    {
        bugf("decompress_player_gzip(): fork failed for '%s'", path);
        return FALSE;
    }

    if (pid == 0)
    {
        execlp("gzip", "gzip", "-dfq", path, (char *)NULL);
        _exit(127);
    }

    if (waitpid(pid, &status, 0) < 0)
    {
        bugf("decompress_player_gzip(): waitpid failed for '%s'", path);
        return FALSE;
    }

    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0)
    {
        bugf("decompress_player_gzip(): gzip failed for '%s'", path);
        return FALSE;
    }
...
bool load_char_obj(DESCRIPTOR_DATA *d, char *name, bool beac, bool equip){
	...
	decompress_player_gzip(strsave);
    file_close(fp);
