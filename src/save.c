...
static bool decompress_player_file(const char *gz_path);
...
static bool decompress_player_file(const char *gz_path)
{
    ...
    if (pid == 0)
    {
        execl("/usr/bin/gzip", "gzip", "-dfq", gz_path, (char *)NULL);
        _exit(127);
    }
...
bool load_char_obj(DESCRIPTOR_DATA *d, char *name, bool beac, bool equip){
    ...
    {
	if (!decompress_player_file(strsave))
	    bugf("Load_char_obj: unable to decompress %s", strsave);
    }
    file_close(fp);
